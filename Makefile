...
test-main:
	...
	# XXX
	WEBSERVER_DEBUG=2 WEBUA_DEBUG=2 TEST_METHOD=HTTPS $(PERL) t/server.t || true
	WEBSERVER_DEBUG=2 WEBUA_DEBUG=2 TEST_METHOD=HTTPS.+with.+server $(PERL) t/server.t || true
	$(PROVE) t/*.t
	$(PROVE) t/real/*.t
