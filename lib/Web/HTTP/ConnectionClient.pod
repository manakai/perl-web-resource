...
The input is XXX.  At minimum, the C<url> argument must be specified.
It is a URL record object (L<Web::URL>).  It must have same origin as
the client's origin (i.e. the origin of the argument to the
C<new_from_url> method).
