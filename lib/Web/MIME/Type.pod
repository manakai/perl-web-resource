...
A C<Web::MIME::Type> object represents a MIME type (aka Internet Media
Type), with or without parameters.

XXX MIME Sniffing Standard is not supported yet.
...
=item I<$t> = Web::MIME::Type->parse_web_mime_type (I<$string>, [I<$onerror>])
...
it is apparently useless in the real world and the C<Web::MIME::Type>
module does not support such unusual parameters.  The parser reports a
warning if there are more than one C<parameter>s with same
C<attribute>.
