...
lowercase, in the order of additions).
...
Serialize the object as a valid MIME type with no parameters in
lowercase.  If the object is I<invalid>, C<undef> is returned instead.

=item $string = $mime->as_valid_mime_type

Serialize the object as a valid MIME type.  If the object is
