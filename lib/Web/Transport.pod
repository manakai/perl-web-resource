...
For the C<request> method of L<Web::Transport::ConnectionClient>
objects and the C<new> class method of L<Web::Transport::WSClient>,
...
The scheme of the URL must be one of C<http>, C<https>, or C<ftp> for
L<Web::Transport::ConnectionClient>, or C<ws> or C<wss> for
...
This option is not available for L<Web::Transport::WSClient>.
...
Specify HTTP headers.  The value must be a hash reference, whose keys
are header names and values are corresponding header values.  If a
value is an array reference, the array's items are used as the header
values (i.e. multiple headers with same name are generated).  Header
...
L<Web::Transport::ConstProxyManager>.
...
Additionally, modules L<Web::Transport::TCPStream> require following
modules (which are also submodules of this Git repository):

=over 4

=item modules from perl-web-dom package <https://github.com/manakai/perl-web-dom>, e.g. L<Web::DOM::Error>

=item modules from perl-streams package <https://github.com/manakai/perl-streams>, e.g. L<ArrayBuffer> and L<DataView>
