...
Specify HTTP headers.  The value must be either a hash reference or an
array reference.

If the value is a hash reference, its keys are header names and its
values are corresponding header values.  If a value is an array
reference, the array's items are used as the header values
(i.e. multiple headers with same name are generated).

If the value is an array reference, it must contain zero or more array
references whose zeroth items are header names and first items are
corresponding header values.

Header names are case-insensitive.  Header names and values must be
byte strings.
...
If this option has non-C<undef> value, the C<body_length> option must
also be specified.  This option is not allowed when C<body> is
specified.

=item body_length => $byte_length
