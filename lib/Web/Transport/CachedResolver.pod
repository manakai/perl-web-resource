...
=item $promise = $resolver->resolve ($host, no_cache => $boolean, debug => $mode)
...
The C<no_cache> argument specifies whether any previously cached
result should be ignored or not.  Note that, even when C<no_cache>
option is specified, the result of this resolution will be cached.

The C<debug> argument specifies the debug mode.  The default value is
0 (not in the debug mode).  See C<WEBUA_DEBUG> section in
