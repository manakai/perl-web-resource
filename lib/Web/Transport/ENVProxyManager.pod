...
=item $promise = $pm->get_proxies ($url)

Get the list of the proxies for the specified URL.  The argument must
be a URL record (L<Web::URL>).

The method returns a promise (L<Promise>), which is to be resolved
with a proxy configuration list (see L<Web::Transport/"PROXY
CONFIGURATIONS">).
