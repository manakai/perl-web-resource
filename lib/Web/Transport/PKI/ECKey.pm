package Web::Transport::PKI::ECKey;
use strict;
use warnings;
our $VERSION = '1.0';
use Net::SSLeay;
...
sub to_net_ssleay_pkey ($) {
  ...
  unless (defined $self->{pkey}) {
    ...
    Net::SSLeay::EVP_PKEY_assign_EC_KEY ($self->{pkey}, $self->{ec})
        or Web::Transport::NetSSLeayError->new_current;
  }
...
sub DESTROY ($) {
  ...
  Net::SSLeay::EC_KEY_free ($self->{ec})
      if defined $self->{ec} and not defined $self->{pkey};
} # DESTROY
