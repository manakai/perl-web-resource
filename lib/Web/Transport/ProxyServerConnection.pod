...
=item $client = $api->client ($url, $client_options)
...
The second argument must be a hash reference of additional client
options (see L<Web::Transport::BasicClient/"CLIENT OPTIONS">) used to
create a client.  By default a set of client options appropriate for
the proxy server is used, taking the C<client> option of the proxy
server's constructor argument into account, but this argument can be
