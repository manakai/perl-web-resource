...
=item $client = $api->client ($url)

Return a client (L<Web::Transport::BasicClient>) that is ready to send
a request.  The argument must be a URL (L<Web::URL> object) of the
...
looks into the connection pool of the proxy server and sets client
options (see L<Web::Transport::BasicClient/"CLIENT OPTIONS">) of the
proxy server appropriately (as specified by the C<client> option of
the proxy server's constructor argument).  Therefore, if a request or
response handler wants to fetch a resource as part of response
construction, this method should be used exclusively rather than other
HTTP client APIs.
