...
my $url = $ENV{TEST_SERVER_URL};
my $mode = $ENV{TEST_SERVER_MODE} || ''; # or 'apache' or 'nginx'
my $root_path = path (__FILE__)->parent->parent->absolute;
my $cmd;
my $cmd_out = '';

if (defined $url) {
  $url = Web::URL->parse_string ($url) || die "Bad |TEST_SERVER_URL| <$url>";
} else {
  my $host = '127.0.0.1';
  my $port = 7823;
  $cmd = Promised::Command->new ([
    $root_path->child ('perl'), $root_path->child ('t_deps/bin/rawserver2.pl'),
    $host, $port,
  ]);
  $cmd->stdout (\$cmd_out);
  $url = Web::URL->parse_string ("http://$host:$port");
} # server
...
my $test_data_path = $root_path->child
...
for my $path ($test_data_path->children (qr/\.dat\z/)) {
  for_each_test $path->relative ($root_path), {
    input => {is_prefixed => 1},
    ...
    } n => 1, name => [$path->relative ($test_data_path), $test->{name}->[0]], timeout => 120;
  };
} # $path

$cmd->run->to_cv->recv if defined $cmd;
(promised_wait_until {
  my $transport = Web::Transport::TCPTransport->new
      (host => $host, port => $port);
  return promised_cleanup {
    return $transport->abort;
  } $transport->start (sub { })->then (sub { return 1 }, sub { return 0 });
})->to_cv->recv;
run_tests;
if (defined $cmd) {
  Promise->resolve->then (sub {
    ...
    return promised_cleanup {
      return $transport->abort;
    } $transport->start (sub { })->then (sub {
      $transport->push_write (\"GET /end\x0D\x0A");
      return $transport->push_shutdown;
    });
  })->then (sub {
    return (promised_wait_until {
      $cmd_out =~ /\x0A\Q[[rawserver end]]\E\x0A/;
    } timeout => 5)->catch (sub { });
  })->to_cv->recv;
  $cmd->send_signal ('INT');
  $cmd->wait->catch (sub { })->to_cv->recv;
}
