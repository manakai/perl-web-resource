...
test {
  my $c = shift;
  eval {
    HTTPConnectionClient->new_from_url;
  };
  ok $@;
  done $c;
...
test {
  ...
  eval {
    HTTPConnectionClient->new_from_url ($url);
  };
  ok $@;
  done $c;
} n => 1, name => 'new_from_url no url';
...
test {
  ...
  $p->then (sub {
    ...
    my $err = $_[0];
    test {
      is $err, "Bad URL origin |null| (|http://test| expected)";
    } $c;
  })->then (sub {
    done $c;
    undef $c;
  });
} n => 2, name => 'request opaque origin';
